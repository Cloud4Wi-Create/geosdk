...
* a [Selection Operation](/api/concepts/resource-selection.md) on the [*Time*](/api/dimensions/time.md) Dimension (`"select.time.intersect"`).
Specifically, the [*Time Segment*](/api/data-models/d-segment/time.md) has to indicate the time instant we are interseted in (in the example below we consider "now").
...
The Response provided by the API will contain only one [*Item*](/api/general-aspects/pagination.md).
