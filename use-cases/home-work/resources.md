...
The body of the request has to contain a [Selection Operation](/api/concepts/resource-selection.md) on the [*Device Base*](/api/dimensions/device-base.md) Dimension.
Specifically, the [*DeviceBase-Segment*](/api/data-models/d-segment/device-base.md) has to contain the set of IDs of the Devices of interest (e.g., `"<D1>"`, `"<D2>"`, etc.).
...
## Response
The Response provided by the API will contain a set of [*Items*](/api/general-aspects/pagination.md) each of which represents a Home/Work Location Resource (same [data model](/api/data-models/resources/platform-created/device-related/home-location.md)).
